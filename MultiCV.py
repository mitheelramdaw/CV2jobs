...
# Function to rank jobs based on CV similarity and suggest the best matching job


def rank_jobs(uploaded_cvs):
    ranked_jobs = []
    suggestions = []  # Store job suggestions for each CV
    for cv in uploaded_cvs:
        ...
        for i, job_desc in enumerate(job_descriptions):
            ...
            # Check if this job description has the highest similarity for this CV
            if similarity_score > max_similarity:
                max_similarity = similarity_score
                best_job = job_desc

        suggestions.append(best_job)

    ranked_jobs = sorted(
        ranked_jobs, key=lambda x: x["similarity_score"], reverse=True)

    return ranked_jobs, suggestions
...
def main():
    ...
    if st.button("Rank Jobs") and uploaded_files:
        ...
        # Calculate similarity and rank jobs, and get job suggestions
        ranked_jobs, suggestions = rank_jobs(uploaded_cvs)

        # Display the ranked jobs and suggestions
        st.subheader("Ranked Jobs and Suggestions:")
        for i, job in enumerate(ranked_jobs):
            st.write(
                f"CV {i + 1}: {job['job_description']} (Similarity: {job['similarity_score']:.2f}%)"
            )
            if i < len(suggestions):
                st.write(f"   Suggested Job: {suggestions[i]}")
